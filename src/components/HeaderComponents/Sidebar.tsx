  ...
  const renderMenu = (items: any, isNested = false, depth = 0) => {
    return (
      <ul className={`${depth + 1} relative flex flex-col ${isNested ? `` : ''}`}>
        {items.map((item: any) => (
          <li key={item.id}
            className={` hover:bg-[white] ${isSubmenuOpen(item.id) ? 'hover:bg-transparent' : 'bg-white'}`}>
            <a
              href={item.url}
            ...
            >
              ...
              ${depth == 1 ? "ml-0 pl-6 md:pl-[40px] text-[12px] md:text-[14px] text-[#000] font-medium bg-[white] w-full hover:bg-[#b2adb0]" : ""}
              ${depth == 2 ? "ml-0 pl-6 md:pl-[60px] text-[12px] md:text-[14px] text-[#000] font-medium bg-[white] w-full hover:bg-[#b2adb0]" : ""}
              ${depth == 3 ? "ml-0 pl-6 md:pl-[80px] text-[12px] md:text-[14px] text-[#000] font-medium bg-[white] w-full hover:bg-[#b2adb0]" : ""}
              ${depth == 4 ? "ml-0 pl-6 md:pl-[80px] text-[12px] md:text-[14px] text-[#000] font-medium bg-[white] w-full hover:bg-[#b2adb0]" : ""}`}
              >
                {item.title}
              </div>
              {item.submenu && (
