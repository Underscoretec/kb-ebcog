...
const CourseFeeCard: React.FC<PlanCardProps> = ({ withoutHotelData, withHotelData, handleProceed }: any) => {
    ...
    return (
        <div className="flex flex-col items-center p-4 sm:p-8 bg-gray-50 h-auto">
            <div className='flex border border-[#EAEAEA] rounded-[20px] overflow-hidden  cursor-pointer font-montserrat text-[20px] font-bold leading-[24px]'>
                <div className={`px-20 py-4 ${activeTab === 'withHotel' ? 'bg-[#E4087F] text-white' : 'text-black'}`} onClick={() => setActiveTab('withHotel')}>With Hotel Accommodation</div>
                <div className={`px-20 py-4 ${activeTab === 'withoutHotel' ? 'bg-[#E4087F] text-white' : 'text-black'}`} onClick={() => setActiveTab('withoutHotel')}>Without Hotel Accommodation</div>
            </div>

            <div className='shadow-custom my-12 p-6 rounded-lg'>
                ...
                {dayjs().isBefore(dayjs(plan?.discount?.endDate)) &&
                    <div className='my-4 py-4 px-20 bg-[#290849] rounded-md text-white'>
