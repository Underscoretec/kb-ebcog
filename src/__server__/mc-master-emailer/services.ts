...
import MMCampaignsModel from "./model";
    ...
    try {
        ...
        if (req?.query?.fileType === 'uploadExcel') {
            ...
            if (modifyCodes?.length > 0) {
                const campaignsList = await MMCampaignsModel.insertMany(modifyCodes, { ordered: false })
                console.log(campaignsList?.length, `MMCampaigns list saved successfully >>>>>>>>`);
                if (campaignsList) {
        ...
        logger.error(error, "[MMCampaigns importData -001] Error in import data from xlsx! ");
...
const sendEmails = async (req: NextApiRequest, res: NextApiResponse) => {
    try {
        logger.info("[MMCampaigns sendEmails -002] sendEmails function called.")
        if (req.query.sync === 'safe') {
            ...
            if (req?.query?.mailProvider === "SMTP" || req?.query?.mailProvider === "SES") {
                ...
                if (batchemails?.batchEmail?.length > 0) {
                    ...
                    const campaignUpdate = await MMCampaignsModel.updateMany({ email: { $in: batchemails?.batchEmail } },
                        {
                            $set: {
                                mailSent: {
                                    status: true,
                    ...
                    for (const [idx, item] of datas.entries()) {
                        ...
                        if (item['email'] && emailStatus === 'safe') {
                            ...
                            if (sendMailArray?.length === 100) {
                                logger.info(`Updating ${sendMailArray?.length} success emails`)

                                const campaignUpdate = await MMCampaignsModel.updateMany({ email: { $in: sendMailArray } },
                                    {
                                        $set: {
                                            mailSent: {
                                                status: true,
        ...
        logger.error(error, "[MMCampaigns sendEmails -002] Error in send emails!")
        return res.status(400).json({
...
const unSubscribeEmail = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    try {
        logger.info("MMCampaigns Call addData function");
        const campaign = await MMCampaignsModel.findOne({ email: email, enabled: 1 })
        if (!campaign) {
        ...
        logger.error(error, "[MMCampaigns  ] Error unSubscribe Campaign! ");
        return res.status(500).json(errorResponse(error));
    }
...
const successEmailUpdate = async (req: NextApiRequest, res: NextApiResponse) => {
    try {
        ...
        if (resultJson?.successSendEmail?.length > 0) {
            ...
            const campaignUpdate = await MMCampaignsModel.updateMany({ email: { $in: resultJson?.successSendEmail } },
                {
                    $set: {
                        mailSent: {
                            status: true,
